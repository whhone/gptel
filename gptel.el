...
(defun gptel--url-parse-response (backend proc-info)
  ...
  (if-let* ((http-msg (string-trim (buffer-substring (line-beginning-position)
      ...
      (if (equal http-status "100")
          (list nil http-status http-msg nil)
        (if-let* ((response (progn (goto-char url-http-end-of-headers)
                                   (condition-case nil
                                       (gptel--json-read)
                                     (error 'json-read-error)))))
            (cond
             ;; FIXME Handle the case where HTTP 100 is followed by HTTP (not 200) BUG #194
             ((or (memq url-http-response-status '(200)) ;; Removed 100
